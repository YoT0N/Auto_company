...
public class BusControllerTest {
    ...
    @Test
    void getBusById_WhenNotExists_ShouldReturnNotFound() throws Exception {
        // Given
        when(busService.getBusById("NONEXISTENT")).thenReturn(Optional.empty());

        // When & Then
        mockMvc.perform(get("/api/buses/NONEXISTENT"))
                .andExpect(status().isNotFound());

        verify(busService, times(1)).getBusById("NONEXISTENT");
    }

    @Test
    void createBus_WithValidData_ShouldCreateAndReturnBus() throws Exception {
        // Given
        when(busService.existsById(anyString())).thenReturn(false);
        when(busService.saveBus(any(BusDTO.class))).thenReturn(testBus1);

        // When & Then
        mockMvc.perform(post("/api/buses")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(testBus1)))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.countryNumber", is("AA1234BB")))
                .andExpect(jsonPath("$.brand", is("Mercedes")));

        verify(busService, times(1)).existsById(testBus1.getCountryNumber());
        verify(busService, times(1)).saveBus(any(BusDTO.class));
    }

    @Test
    void createBus_WithExistingId_ShouldReturnConflict() throws Exception {
        // Given
        when(busService.existsById(testBus1.getCountryNumber())).thenReturn(true);
        ...
        verify(busService, times(1)).existsById(testBus1.getCountryNumber());
        verify(busService, never()).saveBus(any(BusDTO.class));
    }

    @Test
    void updateBus_WhenExists_ShouldUpdateAndReturnBus() throws Exception {
        // Given
        String countryNumber = "AA1234BB";
        BusDTO updatedBus = new BusDTO();
        updatedBus.setCountryNumber(countryNumber);
        updatedBus.setBoardingNumber("B001");
        updatedBus.setBrand("Mercedes-Updated");
        updatedBus.setPassengerCapacity(46);
        updatedBus.setYearOfManufacture(2020);
        updatedBus.setMileage(30000.0);
        updatedBus.setDateOfReceipt(LocalDate.of(2020, 3, 15));
        updatedBus.setWriteoffDate(null);

        when(busService.existsById(countryNumber)).thenReturn(true);
        when(busService.updateBus(eq(countryNumber), any(BusDTO.class))).thenReturn(updatedBus);

        // When & Then
        mockMvc.perform(put("/api/buses/{countryNumber}", countryNumber)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(updatedBus)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.countryNumber", is(countryNumber)))
                .andExpect(jsonPath("$.brand", is("Mercedes-Updated")))
                .andExpect(jsonPath("$.passengerCapacity", is(46)))
                .andExpect(jsonPath("$.mileage", is(30000.0)));

        verify(busService, times(1)).existsById(countryNumber);
        verify(busService, times(1)).updateBus(eq(countryNumber), any(BusDTO.class));
    }

    @Test
    void updateBus_WhenNotExists_ShouldReturnNotFound() throws Exception {
        // Given
        ...
        verify(busService, times(1)).existsById(countryNumber);
        verify(busService, never()).updateBus(anyString(), any(BusDTO.class));
    }
